# 方法论：M4 双向时序记忆 · DACC 密度自适应通道校准

本文档详细说明本仓库两个核心模块的**原理、数学模型与实现机制**，并解释每个设计
决策背后的动机与消融依据。代码位置以 `main` 分支为准。
...
M4 + DACC 叠加后 Score **0.94822**（官方 P0 口径），超过官方 P23 baseline 0.93820。

> 在其之上继续叠加 M5 轨迹外推一致性 loss 的方法论说明见
> **`m4-dacc-m5` 分支**的 `docs/METHODOLOGY.md`（Score 0.94965 / 调优 P0 0.95444）。
...
本分支（`main`）为 M4+DACC 版本，**不含 M5 代码**。M5 是叠加在 M4+DACC 之上的纯训练期
loss，完整的方法论（算法步骤、公式、超参数、为何只在 M4 序列训练上有效）见
**`m4-dacc-m5` 分支的 `docs/METHODOLOGY.md` 第 3 节**。一句话概述：对每个目标用
`target_id` 分组 + 最小二乘线性拟合轨迹，在未观测时间箱的外推位置上施
`relu(margin_logit − logit)` 正则，强化轨迹连续性、抬 Pd（M4+DACC+M5 = 0.94965）。
...
3. **checkpoint metadata**：`best_loss_seed42.pt` 内保存模型架构标志
   （`density_calibration_enabled` 等），推理端 `load_temporal_memory_model` 从
   checkpoint 还原，避免配置与权重失配。
