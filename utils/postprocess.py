...
class P18ScoreTrackRecoveryConfig:
    ...
    The module is deliberately restricted to videos that exceed the training
    event budget.  A retained P0/P0c positive is a seed; only lower-score
    components that form a short, spatially continuous track with such a seed
    can contribute a restored event.  It never uses labels or video names.
    """
...
def _recover_one_video_by_score_tracks(
    ...
    if event_count <= config.event_count_cutoff:
...
class P18ScoreTrackRecovery:
    ...
    def describe(self):
        if not self.enabled:
            return 'disabled'
        return (
            'enabled (event_count > {}, candidate_floor={}, spatial_radius={}, '
